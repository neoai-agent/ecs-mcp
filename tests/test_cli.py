...
from ecs_mcp.client import ECSClientConfig
...
def mock_server():
    ...
    server.client = AsyncMock()  # Use AsyncMock for async methods
    server.run_mcp_blocking = MagicMock()
    return server

@pytest.mark.asyncio
async def test_perform_async_initialization(mock_server):
    """Test async initialization of the server."""
    await perform_async_initialization(mock_server)
    mock_server.client.initialize_ecs.assert_called_once()

def test_main_with_invalid_region(mock_env_vars, mock_server):
    """Test main function with invalid region."""
...
@pytest.mark.parametrize("args,expected_exit,clear_env", [
    ...
    # Test with missing credentials (clear env)
    (['--access-key', 'cli-access-key'], 1, True),
])
...
def test_main_with_invalid_credentials(mock_server):
...
def test_main_with_custom_model(mock_env_vars, mock_server):
    """Test main function with custom model specification."""
    with patch('sys.argv', ['cli.py', '--model', 'custom-model']), \
         patch('ecs_mcp.cli.ECSMCPServer', return_value=mock_server) as mock_server_class, \
         patch('ecs_mcp.cli.anyio.run') as mock_anyio_run:
        with pytest.raises(SystemExit) as exc_info:
            main()
        assert exc_info.value.code == 0
        # Verify server was initialized with custom model
...
def test_main_with_custom_host_port(mock_env_vars, mock_server):
    ...
    with patch('sys.argv', ['cli.py', '--host', '127.0.0.1', '--port', '9000']), \
         patch('ecs_mcp.cli.ECSMCPServer', return_value=mock_server) as mock_server_class, \
         patch('ecs_mcp.cli.anyio.run') as mock_anyio_run:
        with pytest.raises(SystemExit) as exc_info:
            main()
